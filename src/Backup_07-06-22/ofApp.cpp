...
void ofApp::setup() {
	...
	g_hWnd =  ofGetWin32Window(); // WindowFromDC(wglGetCurrentDC());
	...
	//
	// "SpoutCapture" "Window Title"
	//
	// The application window will minimize and capture the window with that title if it exists. 
	// The application will stay minimised but the Spout output will start.
	//
	// If the selected window closes, the application will wait for it to open again.
	// The option is disabled if the user selects another window.
	//
	if (lpCmdLine && lpCmdLine[0]) {
		...
		if (windowHwnd != 0) {
			...
			// Initialize window sender now so it's set as active 
			// rather than desktop capture (see also Update).
			// Capture the window (windowHwnd) in Draw()
			windowSender.CreateSender("WindowSender", ofGetWidth(), ofGetHeight());
			// LJ DEBUG windowSender.SetActiveSender("WindowSender");
			// Minimize for command line
...
void ofApp::exit() {
	...
	// LJ DEBUG
	// MessageBoxA(NULL, "exit()", "ofApp", MB_OK);
...
void ofApp::update() {
	...
	if (!bInitialized) {
		...
		// LJ DEBUG
		if(!windowSender.spout.sendernames.FindSenderName("WindowSender"))
		    windowSender.CreateSender("WindowSender", ofGetWidth(), ofGetHeight());
		...
		// Allow for command line activation of window capture in Setup()
		if(bWindow)
